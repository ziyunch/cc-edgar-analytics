...
def clean_active_log(log_file, ip_dict, out_fh):
    """ evoke inactive sessions log and write to file
    """
    for ip in sorted(log_file, key=lambda x: ip_dict[x][3]):
        ip_dict = output_session(ip, ip_dict, out_fh)
    return ip_dict

def output_session(ip, ip_dict, out_fh):
...
def update_ip(ip_dict, pair, temp, log_index, event_index):
    ...
    if ip not in ip_dict:
        ...
        ip_dict[ip] = [[date, time, dt], [date, time, dt, log_index], 1, event_index]
        temp.add(ip)
    else:
        # Update current session's last update time and event count
        ip_dict[ip][1] = [date, time, dt, log_index]
        ip_dict[ip][2] += 1
        temp.add(ip)
    return ip_dict, temp

def process_weblog(in_fh, out_fh, ip_dict, temp, active_log, inact_period):
    """ Process Weblog
    """
    in_fh.readline()
    ...
    time_count = 0
    event_index = 1
    log_index = time_count % inact_period
    ...
    for line in in_fh:
        ...
        if diff > time_count:
            ...
            ip_dict = clean_active_log(active_log[log_index], ip_dict, out_fh)
            active_log[log_index] = temp
            if ip in ip_dict:
            ...
            log_index = time_count % inact_period
            temp = set()
        ip_dict, temp = update_ip(ip_dict, pair, temp, log_index, event_index)
    # Deal with the remaining ip sessions
    remaindt = set()
    remaindt.update(active_log[log_index])
    active_log[log_index] = temp
    for i in range(0, inact_period):
        remaindt.update(active_log[i])
    ip_dict = clean_active_log(remaindt, ip_dict, out_fh)
...
def main():
    ...
    with open(weblog_file) as in_fh, open(output_file, "a") as out_fh:
        process_weblog(in_fh, out_fh, ip_dict, temp, active_log, inact_period)
